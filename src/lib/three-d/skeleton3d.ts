...
import { buildTimeWarp, clampIndex, type EventRow } from "@/lib/overlay/time-warp";

export type Point3 = { x: number; y: number; z: number; v: number };
export type Pose3 = Point3[];

// Re-exported so existing importers of the 3D module keep working; the
// implementation is shared with the 2D on-video ghost.
export { buildTimeWarp };
