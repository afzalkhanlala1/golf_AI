...
import { emptyFrame, mediapipeToCoco, type Landmark } from "./browser-pose";
 ...
 *  precisely which MediaPipe landmark ended up in which COCO slot. */
...
describe("mediapipeToCoco", () => {
  it("routes each MediaPipe landmark to the correct COCO-17 slot", () => {
    const frame = mediapipeToCoco(tagged(), 100, 100);
    ...
    for (const [cocoIdx, mpIdx] of pairs) {
      // x was set to index/100 then scaled by width 100 → equals the index.
      expect(frame[cocoIdx]!.x, `COCO ${cocoIdx} should come from MP ${mpIdx}`).toBeCloseTo(
        mpIdx,
        6,
      );
    }
  ...
  it("produces exactly 17 keypoints", () => {
    expect(mediapipeToCoco(tagged(), 640, 480)).toHaveLength(17);
  });

  it("scales normalised coordinates into pixel space", () => {
    const lms = tagged();
    lms[MP.leftShoulder] = { x: 0.25, y: 0.5, z: 0, visibility: 0.9 };
    const frame = mediapipeToCoco(lms, 800, 600);
    expect(frame[KP.leftShoulder]!.x).toBeCloseTo(200, 6);
    expect(frame[KP.leftShoulder]!.y).toBeCloseTo(300, 6);
  });

  it("carries visibility through as confidence", () => {
    const lms = tagged();
    lms[MP.leftKnee] = { x: 0.5, y: 0.5, z: 0, visibility: 0.21 };
    const frame = mediapipeToCoco(lms, 100, 100);
    expect(frame[KP.leftKnee]!.c).toBeCloseTo(0.21, 6);
  });

  it("returns a blank frame when no person was detected", () => {
    expect(mediapipeToCoco(undefined, 100, 100)).toEqual(emptyFrame());
    expect(mediapipeToCoco([], 100, 100)).toEqual(emptyFrame());
  });

  it("feeds segmentBodyParts a frame it can fully segment", () => {
    ...
    const frame = mediapipeToCoco(lms, 480, 640);
    const regions = segmentBodyParts(frame);
    // All 11 regions should materialise from a clean MediaPipe detection.
    expect(regions).toHaveLength(11);
    const head = regions.find((r) => r.id === "head")!;
    const shin = regions.find((r) => r.id === "left_shin")!;
    // Head must sit above the shins in image coords (y grows downward).
    expect(head.centroid.y).toBeLessThan(shin.centroid.y);
  });
});
